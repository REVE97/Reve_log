---
title: "[TIL] VUE / VUEX / PINIA" # 포스팅 제목
...
- 프로젝트 생성 : 
1. `npm create vite@latest app_name` -> Vue 선택
2. `npm init vue` - Project name 입력 -> 초기 라이브러리 설치 (Pinia, router 등을 미리 설치할때 사용) 
...
- 중앙 집중화된 상태 관리 기능을 제공(**State + Getters, Actions = Store(중앙 저장소)**)

- 기존 `props`, `emit`, `Provide`, `Inject` 대신 편리하게 사용 가능

- `Store`는 **싱글톤** 처럼 공유된다

- 주로 **전역 로그인 상태**, 여러 페이지에서 공유되는 데이터가 있을 때 사용

### 구성요소

>**State(상태)** : 실제 데이터
**Getters(파생 값)** : 상태로부터 계산되는 값 (computed)
**Actions(상태 변경 함수)** : 상태를 바꾸는 메서드

### 사용 방식

#### Composition API 스타일 (Vue3)

>현재 실무에서는 Vuex 방식 대신 Composition API 스타일로 주로 사용

>**`import { defineStore } from 'pinia'`** 로 사용 -> **`defineStore('스토어명', 함수)`**

>**State, Getters** 를 **구조 분해 할당** 할때 **`import { storeToRefs } from 'pinia'`** 를 사용하는 것을 권장, **Actions** 은 사용 불필요
ex) `const { count, doubleCount } = storeToRefs(counterStore)`

>**스토어명은 중복되지 않게 사용** -> 충돌 발생
>>실무에서는 `defineStore('counter','auth','trip','user')` 와 같이 사용
