...
cover: ./cs.png # 썸네일 이미지
---

## 프론트엔드에서 Axios Method 호출 시 payload, response 확인 
>개발 과정에서 프론트엔드에서 백엔드의 API 서버와 `Axios` + `REST API`를 사용하여 데이터를 통신할 때
데이터 통신이 제대로 처리되고 있는지 `console.log` 를 통해 
로그로 확인이 어려울 때는 웹페이지의 개발자 도구에서 `NETWORK` 탭을 이용해서 확인가능

---

## CORS 관련
>프론트엔드와 백엔드 API 서버와 **Axios Method 를 이용해서 서로 통신**할 때
**서로 포트 번호가 다르기 때문에 (Origin)**
프론트엔드에서는 Proxy를 통해 **CORS 허용**을 해주어야 한다.
...
>**디자인 패턴 / 아키텍처** : 코드를 어떤 책임으로 나누고, 어떤 규칙대로 작성할 것인가

**<예시>**
```
UI는 직접 API를 호출하지 않는다.

API 호출은 api/service 계층에서 한다.

여러 컴포넌트에서 재사용되는 상태는 Store에서 관리한다.

상품 관련 코드는 상품 Feature 안에서 관리한다.
```

>**폴더 구조** : 작성한 코드를 실제 디렉터리 어디에 위치하게 할 것인가
...
### Feature-based Architecture 디자인 패턴

>해당 디자인 패턴의 핵심은 **기능별로 모은다**

- 공통으로 사용하는 컴포넌트( Button, Modal, Header 등)는 `src/components` 에 위치하고 
**기능별로 컴포넌트를 분리**
...
>Feature-based 디자인 패턴 사고를 훨씬 체계적으로 규칙화한 아키텍처 방법론 중 하나
